...
sf::Vector2f Collision::RotatePoint(const sf::Vector2f& Point, float Angle) {
    Angle = (float)(Angle * RADIANS_PER_DEGREE);
    ...
    if (Object1AABB.Intersects(Object2AABB, &Intersection)) {
        ...
        for (int i = Intersection.Left; i < Intersection.Right; i++) {
            for (int j = Intersection.Top; j < Intersection.Bottom; j++) {
                ...
                o1v = (Object1.TransformToLocal(sf::Vector2f((float)i, (float)j))); //Creating Objects each loop :(
                o2v = Object2.TransformToLocal(sf::Vector2f((float)i, (float)j));
...
bool Collision::BoundingBoxTest(const sf::Sprite& Object1, const sf::Sprite& Object2) {
    ...
    float Angle = (float)Object1.GetRotation() - Object2.GetRotation();
    float CosA = (float)cos(Angle * RADIANS_PER_DEGREE);
    float SinA = (float)sin(Angle * RADIANS_PER_DEGREE);
